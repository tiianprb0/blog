    ...
    function generatePreviewHtml() {
        ...
        if (headings.length > 0) {
            ...
            `) {
                ...
                // Naik level (tutup </ul>)
                while (currentLevel > heading.level) {
                    tocHtml += '</ul></li>'; // Tutup </ul> dan </li> dari parent
                    currentLevel--;
                }
            ...
            while (currentLevel > 1) { // Tutup sampai level dasar (misal H2)
                tocHtml += '</li></ul>';
                currentLevel--;
            }
            // Pastikan </ul> terakhir dari toc-list ditutup jika ada <li> yang belum ditutup
            if (tocHtml.endsWith('</li>')) {
                tocHtml = tocHtml.substring(0, tocHtml.length - 5); // Hapus </li> terakhir yang tidak perlu
            }
            tocHtml += `
                        </ul>
                    </div>
