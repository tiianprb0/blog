...
function generatePostHtml($post_data, $author_info) {
    ...
    if (!empty($headings_for_toc)) {
        ...
        foreach ($headings_for_toc as $heading) {
            ...
            $toc_html .= '<li><a href="#' . $heading['id'] . '">' . $heading['text'] . '</a>';
        }
        while ($current_level > 2) { // Tutup tag yang belum tertutup
            $toc_html .= '</li></ul>';
            $current_level--;
        }
        $toc_html .= '
...
<body>
    <header class="blog-header">
        ...
        <div class="breadcrumb">
            <a href="/blog">blog</a> / <a href="/blog/category/{$category}">{$category}</a> / <a href="#">{$title}</a>
        </div>
    </header>
